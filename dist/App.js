    ...
    swaggerDefinition: {
        ...
        components: {
            schemas: {
                event: {
                    type: 'object',
                    required: ['title', 'description', 'date', 'price', 'location', 'status', 'totalTicket', 'availableTicket', 'bookedTicket'],
                    properties: {
                        title: { type: 'string', description: 'Title of the event' },
                        description: { type: 'string', description: 'Detailed description of the event' },
                        createdAt: { type: 'string', format: 'date-time', description: 'Event creation timestamp' },
                        updatedAt: { type: 'string', format: 'date-time', description: 'Last update timestamp of the event' },
                        date: { type: 'string', format: 'date', description: 'Date of the event' },
                        price: { type: 'number', description: 'Ticket price for the event' },
                        location: { type: 'string', description: 'Location where the event is held' },
                        status: { type: 'string', description: 'Status of the event (e.g., active, canceled)' },
                        totalTicket: { type: 'integer', description: 'Total number of tickets available for the event' },
                        availableTicket: { type: 'integer', description: 'Number of tickets still available' },
                        bookedTicket: { type: 'integer', description: 'Number of tickets already booked' },
                        bookieEmail: {
                            type: 'array',
                        ...
                        image: { type: 'string', format: 'uri', description: 'URL to the event image' },
                        id: { type: 'string', format: 'objectId', description: 'Unique identifier for the event' },
                    },
                },
                user: {
                    type: 'object',
                    required: ['fullName', 'email', 'password', 'role', 'emailVerified', 'active'],
                    properties: {
                ...
                booking: {
                    type: 'object',
                    required: ['user', 'event', 'paymentStatus', 'ticketQuantity', 'dateBooked', 'paymentReference', 'receiptUrl'],
                    properties: {
                        user: { $ref: '#/components/schemas/user', description: 'User who made the booking' },
                        event: { $ref: '#/components/schemas/event', description: 'Event being booked' },
                        paymentStatus: { type: 'string', description: 'Payment status of the booking (e.g., pending, confirmed)' },
                        ticketQuantity: { type: 'integer', description: 'Number of tickets booked' },
                        dateBooked: { type: 'string', format: 'date-time', description: 'Date and time the booking was made' },
                        paymentReference: { type: 'string', description: 'Reference code for the payment' },
                        dateConfirmed: { type: 'string', format: 'date-time', description: 'Date and time the payment was confirmed' },
                        receiptUrl: { type: 'string', format: 'uri', description: 'URL to the payment receipt' },
                    },
                },
            },
        },
...
app.use("/api-docs", express_1.default.static("node_modules/swagger-ui-dist/", { index: false }), swagger_ui_express_1.default.serve, swagger_ui_express_1.default.setup(swaggerDocs, {
    customCssUrl: 'https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/4.19.1/swagger-ui.min.css',
}));
